...
use std::{sync::Arc, time::Duration, f32::INFINITY};
...
impl Envelope {
    ...
    pub fn adsr(attack_height: f32, attack_time: f32, decay_time: f32, sustain_height: f32, sustain_time: f32, release_time: f32) -> Envelope {
        Envelope {
            points: Arc::new(vec![
                (0.0, 0.0), 
    ...
    S: Source + Iterator<Item = f32>,
{
    ...
    fn next(&mut self) -> Option<f32> {
        ...
        let (t1, val1) = if index == 0 { (0.0, 0.0) } else { self.envelope.points[index-1] };
    ...
    S: Source + Iterator<Item = f32>,  
{
    fn current_frame_len(&self) -> Option<usize> {
        ...
        let len_2 = ((self.envelope.last_time() as f64 - self.time).max(0.0) / self.source.sample_rate() as f64).ceil() as usize;
    ...
    fn total_duration(&self) -> Option<Duration> {
        let time1 = self.source.total_duration().map(|dur| dur.as_secs_f32()).unwrap_or(INFINITY);
